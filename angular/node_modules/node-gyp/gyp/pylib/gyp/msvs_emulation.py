...
def ExtractSharedMSVSSystemIncludes(configs, generator_flags):
    ...
    for config in configs:
        ...
        if includes:  # Don't insert a msvs_system_include_dirs key if not needed.
            ...
            new_includes = [
                i for i in includes if i not in all_system_includes
            ]
...
class MsvsSettings:
    ...
    def AdjustLibraries(self, libraries):
        ...
        return [
            lib + ".lib" if not lib.lower().endswith(".lib")
    ...
    def _Setting(self,
        ...
        return self._GetAndMunge(self.msvs_settings[config], path, default,
                                 prefix, append, map)
    ...
    def AdjustMidlIncludeDirs(self, midl_include_dirs, config):
        ...
        includes.extend(
            self._Setting(("VCMIDLTool", "AdditionalIncludeDirectories"),
                          config,
                          default=[]))
        return [self.ConvertVSMacros(p, config=config) for p in includes]
    ...
    def _GetAdditionalLibraryDirectories(self, root, config,
                                         gyp_to_build_path):
    ...
    def GetPGDName(self, config, expand_special):
        ...
        if it's not overridden."""
        config = self._TargetConfig(config)
        output_file = self._Setting(("VCLinkerTool", "ProfileGuidedDatabase"),
                                    config)
        if output_file:
            output_file = expand_special(
                self.ConvertVSMacros(output_file, config=config))
    ...
    def GetLdflags(
        ...
        ld("GenerateMapFile",
           map={"true": "/MAP:" + map_file if map_file else "/MAP"})
        ...
        # be on by default.
        if not any("DYNAMICBASE" in flag or flag == "/FIXED"
                   for flag in ldflags):
    ...
    def _GetLdManifestFlags(self, config, name, gyp_to_build_path,
        ...
        config = self._TargetConfig(config)
        enable_uac = self._Setting(("VCLinkerTool", "EnableUAC"),
                                   config,
                                   default="true")
        manifest_files = []
        ...
        if enable_uac == "true":
            ...
            ui_access = self._Setting(("VCLinkerTool", "UACUIAccess"),
                                      config,
                                      default="false")
  ...
  <security>
    <requestedPrivileges>
      <requestedExecutionLevel level='{execution_level_map[execution_level]}' uiAccess='{ui_access}' />
        ...
        manifest_files += self._GetAdditionalManifestFiles(
    ...
    def IsUseLibraryDependencyInputs(self, config):
        ...
        config = self._TargetConfig(config)
        uldi = self._Setting(("VCLinkerTool", "UseLibraryDependencyInputs"),
                             config)
        return uldi == "true"
    ...
    def IsEmbedManifest(self, config):
        ...
        config = self._TargetConfig(config)
        embed = self._Setting(("VCManifestTool", "EmbedManifest"),
                              config,
                              default="true")
        return embed == "true"
    ...
    RuleShellFlags = collections.namedtuple("RuleShellFlags",
                                            ["cygwin", "quote"])
    ...
    def _HasExplicitRuleForExtension(self, spec, extension):
        ...
        return any(rule["extension"] == extension
    ...
    def HasExplicitIdlRulesOrActions(self, spec):
        ...
        return self._HasExplicitRuleForExtension(
            spec, "idl") or self._HasExplicitIdlActions(spec)
    ...
    def GetIdlBuildData(self, source, config):
        ...
        config = self._TargetConfig(config)
        midl_get = self._GetWrapper(self, self.msvs_settings[config],
                                    "VCMIDLTool")
...
def _LanguageMatchesForPch(source_ext, pch_source_ext):
    c_exts = (".c", )
    cc_exts = (".cc", ".cxx", ".cpp")
    return (source_ext in c_exts
            and pch_source_ext in c_exts) or (source_ext in cc_exts
                                              and pch_source_ext in cc_exts)
...
class PrecompiledHeader:
    ...
    def GetFlagsModifications(self, input, output, implicit, command, cflags_c,
                              cflags_cc, expand_special):
        ...
        if input == self.pch_source:
            ...
            if command == "cxx":
                return (
                    [("cflags_cc", map(expand_special,
                                       cflags_cc + pch_output))],
                    self.output_obj,
                    [],
                )
            elif command == "cc":
...
def GenerateEnvironmentFiles(toplevel_build_dir, generator_flags,
    ...
    for arch in archs:
        ...
        f = open_out(os.path.join(toplevel_build_dir, "environment." + arch),
                     "w")
        f.write(env_block)
        f.close()
        ...
        args = vs.SetupScript(arch)
        args.extend(("&&", "for", "%i", "in", "(cl.exe)", "do", "@echo",
                     "LOC:%~$PATH:i"))
...
def VerifyMissingSources(sources, build_dir, generator_flags, gyp_to_ninja):
    ...
    if int(generator_flags.get("msvs_error_on_missing_sources", 0)):
        ...
        relative = [
            os.path.join(build_dir, gyp_to_ninja(s)) for s in no_specials
        ]
