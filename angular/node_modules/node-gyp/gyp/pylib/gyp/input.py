        ...
        for key, value in zip(node.keys, node.values):
            ...
            if key in dict:
                raise GypError("Key '" + key + "' repeated at level " +
                               ...
                               ".".join(keypath) + "'")
            kp = list(
                keypath)  # Make a copy of the list for descending this node.
            kp.append(key)
...
def LoadOneBuildFile(build_file_path, data, aux_data, includes, is_target,
    ...
    if "skip_includes" not in build_file_data or not build_file_data[
            "skip_includes"]:
        try:
            if is_target:
                LoadBuildFileIncludesIntoDict(build_file_data, build_file_path,
                                              data, aux_data, includes, check)
            else:
                LoadBuildFileIncludesIntoDict(build_file_data, build_file_path,
                                              data, aux_data, None, check)
        except Exception as e:
...
def LoadTargetBuildFile(
    ...
    gyp.DebugOutput(gyp.DEBUG_INCLUDES, "Loading Target Build File '%s'",
                    build_file_path)
    ...
    ProcessVariablesAndConditionsInDict(build_file_data, PHASE_EARLY,
                                        variables, build_file_path)
    ...
    if "target_defaults" in build_file_data:
        ...
        while index < len(build_file_data["targets"]):
            ...
            MergeDicts(new_target_dict, old_target_dict, build_file_path,
                       build_file_path)
...
early_variable_re = re.compile(r"(?P<replace>(?P<type><(?:(?:!?@?)|\|)?)"
                               r"(?P<command_string>[-a-zA-Z0-9_.]+)?"
                               r"\((?P<is_array>\s*\[?)"
                               r"(?P<content>.*?)(\]?)\))")

# This matches the same as early_variable_re, but with '>' instead of '<'.
late_variable_re = re.compile(r"(?P<replace>(?P<type>>(?:(?:!?@?)|\|)?)"
                              r"(?P<command_string>[-a-zA-Z0-9_.]+)?"
                              r"\((?P<is_array>\s*\[?)"
                              r"(?P<content>.*?)(\]?)\))")

# This matches the same as early_variable_re, but with '^' instead of '<'.
latelate_variable_re = re.compile(r"(?P<replace>(?P<type>[\^](?:(?:!?@?)|\|)?)"
                                  r"(?P<command_string>[-a-zA-Z0-9_.]+)?"
                                  r"\((?P<is_array>\s*\[?)"
                                  r"(?P<content>.*?)(\]?)\))")
...
def ExpandVariables(input, phase, variables, build_file):
    ...
    for match_group in matches:
        ...
        if file_list:
            ...
            contents = ExpandVariables(contents, phase, processed_variables,
                                       build_file)
        else:
            # Recurse to expand variables in the contents
        ...
        if file_list:
            contents_list = contents if type(
                contents) is list else contents.split(" ")
            replacement = contents_list[0]
            if os.path.isabs(replacement):
                raise GypError('| cannot handle absolute paths, got "%s"' %
                               replacement)
            ...
            if not generator_filelist_paths:
                ...
                qualified_out_dir = generator_filelist_paths[
                    "qualified_out_dir"]
                path = os.path.join(qualified_out_dir, rel_build_file_dir,
                                    replacement)
            ...
            if cached_value is None:
                ...
                if command_string == "pymod_do_main":
                    ...
                    try:
                        ...
                        result = subprocess.run(contents,
            ...
            if contents not in variables:
                if contents[-1] in ["!", "/"]:
                    ...
                    raise GypError("Undefined variable " + contents + " in " +
                                   build_file)
            else:
            ...
            for item in replacement:
                ...
                if not contents[-1] == "/" and type(item) not in (str, int):
                    raise GypError(
                        ...
                        "list contains a " + item.__class__.__name__)
    ...
    if output == input:
        ...
        gyp.DebugOutput(gyp.DEBUG_VARIABLES, "Found output %r, recursing.",
                        output)
  ...
  that nothing should be used."""
    ...
    while i < len(condition):
        ...
        if len(condition) > i + 2 and type(condition[i + 2]) is dict:
            ...
            i = i + 2
        if result is None:
            result = EvalSingleCondition(cond_expr, true_dict, false_dict,
                                         phase, variables, build_file)

    return result


def EvalSingleCondition(cond_expr, true_dict, false_dict, phase, variables,
                        build_file):
...
def ProcessConditionsInDict(the_dict, phase, variables, build_file):
    ...
    for condition in conditions_list:
        ...
        if merge_dict is not None:
            ...
            # merging it.
            ProcessVariablesAndConditionsInDict(merge_dict, phase, variables,
                                                build_file)
...
def ProcessVariablesAndConditionsInDict(the_dict,
                                        phase,
                                        variables_in,
                                        build_file,
                                        the_dict_key=None):
    ...
    for key, value in the_dict.items():
        ...
        if key != "variables" and type(value) is str:
            ...
            if type(expanded) not in (str, int):
                raise ValueError(
                    "Variable expansion in this context permits str and int " +
                    "only, found " + expanded.__class__.__name__ + " for " +
                    key)
...
def ProcessVariablesAndConditionsInList(the_list, phase, variables,
    ...
    while index < len(the_list):
        ...
        if type(item) is dict:
            ...
            ProcessVariablesAndConditionsInList(item, phase, variables,
                                                build_file)
    ...
    for build_file in data["target_build_files"]:
        for target in data[build_file].get("targets", []):
            ...
            if target_name in targets:
                raise GypError("Duplicate target definitions for " +
                               target_name)
    ...
    for target, target_dict in targets.items():
        ...
        for dependency_key in all_dependency_sections:
            ...
            for index, dep in enumerate(dependencies):
                ...
                if (dependency_key != "dependencies"
                        and dependency not in target_dict["dependencies"]):
                    raise GypError("Found " + dependency + " in " +
                                   dependency_key + " of " + target +
        ...
        for dependency_key in dependency_sections:
            ...
            while index < len(dependencies):
                ...
                if dependency_build_file == target_build_file:
                    ...
                    raise GypError("Found wildcard in " + dependency_key +
                                   " of " + target +
        ...
        for dependency_key in dependency_sections:
            ...
            if dependencies:
                for t in dependencies:
                    if t == target_name and (targets[t].get(
                            "variables", {}).get("prune_self_dependency", 0)):
            ...
            if dependencies:
                for t in dependencies:
                    if target_dict.get("type", None) == "none":
                        if targets[t].get("variables",
                                          {}).get("link_dependency", 0):
                            target_dict[dependency_key] = Filter(
...
class DependencyGraphNode:
  ...
  """
    ...
    def _LinkDependenciesInternal(self,
                                  targets,
    ...
    """
        ...
        # are handling.
        if (not initial and target_type == "shared_library"
    ...
    """
        ...
        return self._LinkDependenciesInternal(targets,
                                              include_shared_libraries)
    ...
    for target, spec in targets.items():
        ...
        for dependency in target_dependencies:
            try:
                ...
                gyp.common.ExceptionAppend(
                    e, "while computing dependencies of .gyp file %s" %
                    build_file)
                raise
    ...
    for target in flat_list:
        ...
        if key == "all_dependent_settings":
            ...
            dependencies = dependency_nodes[
                target].DirectAndImportedDependencies(targets)
        elif key == "link_settings":
            dependencies = dependency_nodes[
                target].DependenciesForLinkSettings(targets)
        else:
        ...
        for dependency in dependencies:
            ...
            if key not in dependency_dict:
                continue
            dependency_build_file = gyp.common.BuildFile(dependency)
            MergeDicts(target_dict, dependency_dict[key], build_file,
                       dependency_build_file)
...
def AdjustStaticLibraryDependencies(flat_list, targets, dependency_nodes,
    ...
    for target in flat_list:
        ...
        if target_type == "static_library":
            ...
            link_dependencies = dependency_nodes[
                target].DependenciesToLinkAgainst(targets)
            ...
            if sort_dependencies and "dependencies" in target_dict:
                target_dict["dependencies"] = [
                    dep for dep in reversed(flat_list)
...
def MergeLists(to, fro, to_file, fro_file, is_paths=False, append=True):
    ...
    for item in fro:
        ...
        if type(item) in (str, int):
            ...
            to_item = MakePathRelative(to_file, fro_file,
                                       item) if is_paths else item
            ...
            raise TypeError("Attempt to merge list item of unsupported type " +
                            item.__class__.__name__)

        if append:
            # If appending a singleton that's already in the list, don't append.
            # This ensures that the earliest occurrence of the item will stay put.
            if not singleton or not is_in_set_or_list(to_item, hashable_to_set,
                                                      to):
                to.append(to_item)
...
def MergeDicts(to, fro, to_file, fro_file):
    ...
    for k, v in fro.items():
        ...
        if k in to:
            ...
            if bad_merge:
                raise TypeError("Attempt to merge dict value of type " +
                                v.__class__.__name__ +
                                " into incompatible type " +
                                to[k].__class__.__name__ + " for key " + k)
        if type(v) in (str, int):
            ...
            for list_incompatible in lists_incompatible:
                if list_incompatible in fro:
                    raise GypError("Incompatible list policies " + k +
                                   " and " + list_incompatible)
...
def MergeConfigWithInheritance(new_configuration_dict, build_file, target_dict,
    ...
    MergeDicts(new_configuration_dict, configuration_dict, build_file,
               build_file)
...
def SetUpConfigurations(target, target_dict):
    ...
    if "default_configuration" not in target_dict:
        concrete = [
            i for (i, config) in target_dict["configurations"].items()
    ...
    target_dict["configurations"] = {
        k: v
        for k, v in configs.items() if not v.get("abstract")
    }
...
def Load(
    ...
    if parallel:
        LoadTargetBuildFilesParallel(build_files, data, variables, includes,
                                     depth, check, generator_input_info)
    else:
        aux_data = {}
        for build_file in build_files:
            try:
                LoadTargetBuildFile(build_file, data, aux_data, variables,
                                    includes, depth, check, True)
            except Exception as e:
                gyp.common.ExceptionAppend(
    ...
    if root_targets:
        ...
        targets, flat_list = PruneUnwantedTargets(targets, flat_list,
                                                  dependency_nodes,
                                                  root_targets, data)
    ...
    for settings_type in [
        ...
        DoDependentSettings(settings_type, flat_list, targets,
                            dependency_nodes)
    ...
    for target in flat_list:
        target_dict = targets[target]
        build_file = gyp.common.BuildFile(target)
        ProcessVariablesAndConditionsInDict(target_dict, PHASE_LATE, variables,
                                            build_file)
    ...
    for target in flat_list:
        target_dict = targets[target]
        build_file = gyp.common.BuildFile(target)
        ProcessVariablesAndConditionsInDict(target_dict, PHASE_LATELATE,
