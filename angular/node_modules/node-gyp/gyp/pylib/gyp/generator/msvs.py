...
def _ConvertSourcesToFilterHierarchy(sources,
                                     prefix=None,
                                     excluded=None,
                                     list_excluded=True,
                                     msvs_version=None):
...
def _ConfigWindowsTargetPlatformVersion(config_data, version):
    ...
    for ver in vers:
        for key in [
            ...
            names = sorted(
                (x for x in (os.listdir(expected_sdk_dir) if os.path.isdir(
                    expected_sdk_dir) else []) if x.startswith(version)),
                reverse=True,
            )
            if names:
                return names[0]
...
def _BuildCommandLineForRuleRaw(spec, cmd, cygwin_shell, has_input_path,
    ...
    if cygwin_shell:
        ...
        direct_cmd = cmd
        direct_cmd = [
            i.replace("$(IntDir)", '`cygpath -m "${INTDIR}"`')
            for i in direct_cmd
        ]
        direct_cmd = [
            i.replace("$(OutDir)", '`cygpath -m "${OUTDIR}"`')
            for i in direct_cmd
        ]
        direct_cmd = [
            i.replace("$(InputDir)", '`cygpath -m "${INPUTDIR}"`')
            for i in direct_cmd
        ]
        if has_input_path:
            direct_cmd = [
        ...
        direct_cmd = [
            '\\"%s\\"' % i.replace('"', '\\\\\\"') for i in direct_cmd
        ]
...
def _BuildCommandLineForRule(spec, rule, has_input_path, do_setup_env):
    ...
    return _BuildCommandLineForRuleRaw(spec,
                                       rule["action"],
                                       mcs,
                                       has_input_path,
                                       quote_cmd,
                                       do_setup_env=do_setup_env)
  ...
  """
    ...
    for config_name, c_data in spec["configurations"].items():
        p.AddFileConfig(_FixPath(primary_input),
                        _ConfigFullName(config_name, c_data),
                        tools=[tool])
  ...
  """
    ...
    for r in rules:
        ...
        if "action" not in r and not r.get("rule_sources", []):
            continue
        cmd = _BuildCommandLineForRule(spec,
                                       r,
                                       has_input_path=True,
                                       do_setup_env=True)
...
def _GenerateExternalRules(rules, output_dir, spec, sources, options,
                           actions_to_add):
...
def _GenerateRulesForMSVS(p, output_dir, options, spec, sources,
  ...
  """
    ...
    rules_native = [
        r for r in rules if not int(r.get("msvs_external_rule", 0))
    ]
    ...
    if rules_external:
        _GenerateExternalRules(rules_external, output_dir, spec, sources,
                               options, actions_to_add)
...
def _GenerateProject(project, options, version, generator_flags, spec):
  ...
  """
    ...
    if version.UsesVcxproj():
        return _GenerateMSBuildProject(project, options, version,
...
def _GenerateMSVSProject(project, options, version, generator_flags):
  ...
  """
    ...
    for config_name, config in spec["configurations"].items():
        _AddConfigurationToMSVSProject(p, spec, config_type, config_name,
                                       config)
    ...
    # Add rules.
    actions_to_add = {}
    _GenerateRulesForMSVS(p, project_dir, options, spec, sources,
                          excluded_sources, actions_to_add)
...
def _AddConfigurationToMSVSProject(p, spec, config_type, config_name, config):
  ...
  """
    ...
    _ToolAppend(tools, "VCLinkerTool", "AdditionalDependencies",
                ["$(NOINHERIT)"])
    ...
    _ToolAppend(tools, "VCCLCompilerTool", "AdditionalIncludeDirectories",
                include_dirs)
    _ToolAppend(tools, "VCMIDLTool", "AdditionalIncludeDirectories",
                midl_include_dirs)
    _ToolAppend(
        tools,
    ...
    _ToolAppend(tools, "VCCLCompilerTool", "DisableSpecificWarnings",
                disabled_warnings)
    # Add Pre-build.
    ...
    if precompiled_header:
        ...
        _ToolAppend(tools, "VCCLCompilerTool", "PrecompiledHeaderThrough",
                    precompiled_header)
        _ToolAppend(tools, "VCCLCompilerTool", "ForcedIncludeFiles",
                    precompiled_header)
...
def _GetDefines(config):
  ...
  """
    ...
    for d in config.get("defines", []):
        fd = "=".join([str(dpart)
                       for dpart in d]) if isinstance(d, list) else str(d)
        defines.append(fd)
    return defines
  ...
  """
    ...
    for tool, settings in tools.items():
        ...
        for setting, value in settings.items():
            if type(value) == list:
                if (tool == "VCLinkerTool"
                        and setting == "AdditionalDependencies"
...
def _AdjustSourcesAndConvertToFilterHierarchy(spec, options, gyp_dir, sources,
                                              excluded_sources, list_excluded,
                                              version):
...
def _IdlFilesHandledNonNatively(spec, sources):
    ...
    for rule in spec.get("rules", []):
        if rule["extension"] == "idl" and int(rule.get("msvs_external_rule",
                                                       0)):
            using_idl = True
            break
    excluded_idl = [i for i in sources
                    if i.endswith(".idl")] if using_idl else []
    return excluded_idl
...
def _ExcludeFilesFromBeingBuilt(p, spec, excluded_sources, excluded_idl,
    ...
    for file_name, excluded_configs in exclusions.items():
        if not list_excluded and len(excluded_configs) == len(
                spec["configurations"]):
    ...
    for cpy in spec.get("copies", []):
        for src in cpy.get("files", []):
            ...
            if src.endswith("/"):
                ...
                cmd = (f'mkdir "{fix_dst}" 2>nul & set ERRORLEVEL=0 & '
                       f'copy /Y "{_FixPath(src)}" "{_FixPath(dst)}"')
                copies.append(
                    ([src], [dst], cmd, f"Copying {src} to {fix_dst}"))
    return copies
...
def _CollapseSingles(parent, node):
    ...
    if type(node) == dict and len(node) == 1 and next(
            iter(node)) == parent + ".vcproj":
    ...
    for qualified_target in target_list:
        ...
        if not spec.get("msvs_external_builder_out_dir"):
            ...
            spec["msvs_external_builder_out_dir"] = os.path.join(
                gyp.common.RelativePath(params["options"].toplevel_dir,
                                        gyp_dir),
...
def CalculateVariables(default_variables, params):
    ...
    if (os.environ.get("PROCESSOR_ARCHITECTURE", "").find("64") >= 0
...
def GenerateOutput(target_list, target_dicts, data, params):
  ...
  """
    ...
    (target_list,
     target_dicts) = MSVSUtil.ShardTargets(target_list, target_dicts)

    # Optionally use the large PDB workaround for targets marked with
    # 'msvs_large_pdb': 1.
    (target_list,
     target_dicts) = MSVSUtil.InsertLargePdbShims(target_list, target_dicts,
                                                  generator_default_variables)
    ...
    for build_file in data:
        ...
        root_entries = _GatherSolutionFolders(sln_projects,
                                              project_objects,
                                              flat=msvs_version.FlatSolution())
        # Create solution.
...
def _GenerateMSBuildFiltersFile(
  ...
  """
    ...
    if filter_group:
        ...
        easy_xml.WriteXmlIfChanged(content,
                                   filters_path,
                                   pretty=True,
                                   win32=True)
...
def _MapFileToMsBuildSourceType(source, rule_dependencies,
                                extension_to_rule_name, platforms, toolset):
...
class MSBuildRule:
  ...
  """
    ...
    def __init__(self, rule, spec):
        ...
        old_command = _BuildCommandLineForRule(spec,
                                               rule,
                                               has_input_path=True,
                                               do_setup_env=True)
    ...
    for rule in msbuild_rules:
        ...
        read_tlog_section = [
            "WriteLinesToFile",
            {
                ...
                "Lines":
                f"^%({rule.tlog}.Source);%({rule.tlog}.Inputs)",
            },
        ]
        command_and_input_section = [
            rule_name,
            {
                ...
                "High",
                "CommandLineTemplate":
                "%%(%s.CommandLineTemplate)" % rule_name,
                "AdditionalOptions":
                "%%(%s.AdditionalOptions)" % rule_name,
                "Inputs":
                rule_inputs,
            },
        ]
        content.extend([
            [
                "Target",
                {
                    ...
                    rule.target_name,
                    "BeforeTargets":
                    "$(%s)" % rule.before_targets,
                    "AfterTargets":
                    "$(%s)" % rule.after_targets,
                    "Condition":
                    "'@(%s)' != ''" % rule_name,
                    "DependsOnTargets":
                    "$(%s);%s" % (rule.depends_on, rule.compute_output),
                    "Outputs":
                    target_outputs,
                    "Inputs":
                    target_inputs,
...
def _GetMSBuildProjectConfigurations(configurations, spec):
    ...
    for (name, settings) in sorted(configurations.items()):
        configuration, platform = _GetConfigurationAndPlatform(
            name, settings, spec)
...
def _GetMSBuildGlobalProperties(spec, version, guid, gyp_file_name):
    ...
    if spec.get("msvs_enable_winrt"):
        ...
        if spec.get("msvs_application_type_revision"):
            ...
            properties[0].append(
                ["ApplicationTypeRevision", app_type_revision])
        else:
    ...
    for configuration in spec["configurations"].values():
        ...
        msvs_windows_sdk_version = (msvs_windows_sdk_version
                                    or _ConfigWindowsTargetPlatformVersion(
...
def _GetMSBuildConfigurationDetails(spec, build_file):
    ...
    for name, settings in spec["configurations"].items():
        ...
        if vctools_version and "msvs_enable_winrt" not in spec:
            _AddConditionalProperty(properties, condition, "VCToolsVersion",
                                    vctools_version)
...
def _ConvertMSVSBuildAttributes(spec, config, build_file):
    ...
    for a in msvs_attributes:
        if a in ["IntermediateDirectory", "OutputDirectory"]:
            ...
            msbuild_attributes[a] = _ConvertMSVSCharacterSet(
                msvs_attributes[a])
...
def _GetMSBuildConfigurationGlobalProperties(spec, configurations, build_file):
    ...
    for (name, configuration) in sorted(configurations.items()):
        ...
        for name, value in sorted(tool_settings.items()):
            ...
            _AddConditionalProperty(properties, condition, name,
                                    formatted_value)
...
def _FinalizeMSBuildSettings(spec, configuration):
        ...
        for ignored_setting in ignored_settings:
            ...
            if value:
                print(
                    ...
                    "%s.  Ignoring setting of %s" %
                    (ignored_setting, str(value)))
    ...
    _ToolAppend(msbuild_settings, "ClCompile", "AdditionalIncludeDirectories",
                include_dirs)
    _ToolAppend(msbuild_settings, "Midl", "AdditionalIncludeDirectories",
                midl_include_dirs)
    _ToolAppend(
        msbuild_settings,
        "ResourceCompile",
    ...
    if target_ext:
        _ToolAppend(msbuild_settings,
                    ...
                    only_if_unset=True)
    # Add defines.
    _ToolAppend(msbuild_settings, "ClCompile", "PreprocessorDefinitions",
                defines)
    _ToolAppend(msbuild_settings, "ResourceCompile", "PreprocessorDefinitions",
                defines)
    # Add disabled warnings.
    _ToolAppend(msbuild_settings, "ClCompile", "DisableSpecificWarnings",
                disabled_warnings)
    # Turn on precompiled headers if appropriate.
    if precompiled_header:
        precompiled_header = os.path.split(precompiled_header)[1]
        _ToolAppend(msbuild_settings, "ClCompile", "PrecompiledHeader", "Use")
        _ToolAppend(msbuild_settings, "ClCompile", "PrecompiledHeaderFile",
                    precompiled_header)
        _ToolAppend(msbuild_settings, "ClCompile", "ForcedIncludeFiles",
                    [precompiled_header])
    else:
        _ToolAppend(msbuild_settings, "ClCompile", "PrecompiledHeader",
                    "NotUsing")
        ...
        if isinstance(source, MSVSProject.Filter):
            ...
            if source not in sources_handled_by_action:
                ...
                if len(excluded_configurations) == len(spec["configurations"]):
                    ...
                    for config_name, configuration in sorted(
                ...
                if group == "compile" and not os.path.isabs(source):
                    ...
                    if file_name.startswith("..\\"):
                        ...
                        file_name = re.sub(r"^\$\([^)]+\)\\", "", file_name)
                    detail.append(
                        ["ObjectFileName", "$(IntDir)\\" + file_name])
                grouped_sources[group].append([element, {
                    "Include": source
                }] + detail)
...
def _GetMSBuildProjectReferences(project):
    ...
    if project.dependencies:
        ...
        for dependency in project.dependencies:
            ...
            project_dir = os.path.split(project.path)[0]
            relative_path = gyp.common.RelativePath(dependency.path,
                                                    project_dir)
            project_ref = [
                "ProjectReference",
                {
...
def _GetMSBuildExternalBuilderTargets(spec):
  ...
  """
    build_cmd = _BuildCommandLineForRuleRaw(
        spec, spec["msvs_external_builder_build_cmd"], False, False, False,
        False)
    build_target = ["Target", {"Name": "Build"}]
    build_target.append(["Exec", {"Command": build_cmd}])

    clean_cmd = _BuildCommandLineForRuleRaw(
        spec, spec["msvs_external_builder_clean_cmd"], False, False, False,
        False)
  ...
  """
    ...
    for primary_input, actions in actions_to_add.items():
        ...
        for action in actions:
            ...
            if generator_supports_multiple_toolsets:
                action["inputs"] = [
                    fixup_host_exe(i) for i in action["inputs"]
                ]
            inputs.update(OrderedSet(action["inputs"]))
            outputs.update(OrderedSet(action["outputs"]))
            descriptions.append(action["description"])
...
def _AddMSBuildAction(
    ...
    additional_inputs = ";".join(
        [i for i in inputs_array if i != primary_input])
