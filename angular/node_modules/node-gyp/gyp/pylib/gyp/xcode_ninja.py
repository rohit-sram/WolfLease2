...
def _TargetFromSpec(old_spec, params):
    ...
    if "configurations" in old_spec:
        for config in old_spec["configurations"]:
            ...
            if "IPHONEOS_DEPLOYMENT_TARGET" in old_xcode_settings:
                ...
                new_xcode_settings[
                    "IPHONEOS_DEPLOYMENT_TARGET"] = old_xcode_settings[
            ...
            ninja_target["configurations"][config][
                "xcode_settings"] = new_xcode_settings

    ninja_target["mac_bundle"] = old_spec.get("mac_bundle", 0)
    ninja_target["mac_xctest_bundle"] = old_spec.get("mac_xctest_bundle", 0)
    ninja_target["ios_app_extension"] = old_spec.get("ios_app_extension", 0)
    ninja_target["ios_watchkit_extension"] = old_spec.get(
        "ios_watchkit_extension", 0)
    ...
    if ninja_toplevel:
        ninja_target["actions"] = [
            {
                "action_name":
                "Compile and copy %s via ninja" % target_name,
                "inputs": [],
                "outputs": [],
                "action": [
                    "env",
...
def IsValidTargetForWrapper(target_extras, executable_target_pattern, spec):
  ...
  """
    ...
    if int(spec.get(
            "mac_xctest_bundle",
            0)) != 0 or (spec.get("type", "") == "executable"
...
def CreateWrapper(target_list, target_dicts, data, params):
  ...
  """
    ...
    new_data[main_gyp]["xcode_settings"] = data[orig_gyp].get(
        "xcode_settings", {})
    ...
    for old_qualified_target in target_list:
        ...
        if IsValidTargetForWrapper(target_extras, executable_target_pattern,
                                   spec):
            ...
            # Add to new_data.
            for old_target in data[old_qualified_target.split(":")
                                   [0]]["targets"]:
    ...
    for target, target_dict in target_dicts.items():
        ...
        sources += [
            os.path.relpath(os.path.join(base, file), relative_path)
            for file in files
        ]
    ...
    new_data[sources_gyp]["xcode_settings"] = data[orig_gyp].get(
        "xcode_settings", {})
