...
def CalculateGeneratorInputInfo(params):
    ...
    output_dir = params["options"].generator_output or params[
...
def Compilable(filename):
    ...
    return any(res for res in (filename.endswith(e)
                               for e in COMPILABLE_EXTENSIONS))
...
""" % (ext, COMPILABLE_EXTENSIONS[ext]))
    ...
    def Write(self, qualified_target, base_path, output_filename, spec,
              configs, part_of_all):
        ...
        # Bundle resources.
        if self.is_mac_bundle:
            all_mac_bundle_resources = (spec.get("mac_bundle_resources", []) +
                                        extra_mac_bundle_resources)
    ...
    def WriteActions(
        ...
        for action in actions:
            name = StringToMakefileVariable("{}_{}".format(
                self.qualified_target, action["action_name"]))
            ...
            if "message" in action:
                self.WriteLn("quiet_cmd_{} = ACTION {} $@".format(
                    name, action["message"]))
            else:
            ...
            outputs = [
                gyp.xcode_emulation.ExpandEnvVars(o, env) for o in outputs
            ]
            inputs = [
                gyp.xcode_emulation.ExpandEnvVars(i, env) for i in inputs
            ]
            ...
            self.WriteLn("{} := {}".format(outputs_variable,
                                           " ".join(outputs)))
    ...
    def WriteRules(
        ...
        for rule in rules:
            name = StringToMakefileVariable("{}_{}".format(
                self.qualified_target, rule["rule_name"]))
            count = 0
            ...
            for rule_source in rule.get("rule_sources", []):
                ...
                (rule_source_root,
                 rule_source_ext) = os.path.splitext(rule_source_basename)
                ...
                for output in outputs:
                    ...
                    assert " " not in output, (
                        "Spaces in rule filenames not yet supported (%s)" %
                        output)
                self.WriteLn("all_deps += %s" % " ".join(outputs))

                action = [
                    self.ExpandInputRoot(ac, rule_source_root,
                                         rule_source_dirname)
    ...
    def WriteCopies(self, copies, extra_outputs, part_of_all):
        ...
        for copy in copies:
            for path in copy["files"]:
                ...
                output = Sourceify(
                    self.Absolutify(os.path.join(copy["destination"],
                                                 filename)))
        ...
        self.WriteLn("{} = {}".format(
            variable, " ".join(QuoteSpaces(o) for o in outputs)))
    ...
    def WriteMacBundleResources(self, resources, bundle_deps):
        ...
        for output, res in gyp.xcode_emulation.GetMacBundleResources(
            ...
            if ext != ".xcassets":
                ...
                self.WriteDoCmd([output], [res],
                                "mac_tool,,,copy-bundle-resource",
                                part_of_all=True)
    ...
    def WriteMacInfoPlist(self, bundle_deps):
        ...
        self.WriteDoCmd([out], [info_plist],
                        "mac_tool,,,copy-info-plist",
                        part_of_all=True)
...
# See "Target-specific Variable Values" in the GNU Make manual.""")
    ...
    def WritePchTargets(self, pch_commands):
        ...
        for gch, lang_flag, lang, input in pch_commands:
            ...
            self.WriteLn(f"{gch}: {var_name} := {lang_flag} " +
                         "$(DEFS_$(BUILDTYPE)) "
                         "$(INCS_$(BUILDTYPE)) "
    ...
    def _InstallImmediately(self):
        return (self.toolset == "target" and self.flavor == "mac"
                and self.type in ("static_library", "executable",
    ...
    def ComputeDeps(self, spec):
        ...
        if "dependencies" in spec:
            deps.extend([
                target_outputs[dep] for dep in spec["dependencies"]
    ...
    def WriteTarget(self, spec, configs, deps, link_deps, bundle_deps,
        ...
        if extra_outputs:
            self.WriteDependencyOnExtraOutputs(self.output_binary,
                                               extra_outputs)
            self.WriteMakeRule(
                extra_outputs,
                deps,
            ...
            for configname in sorted(configs.keys()):
                ...
                if self.flavor == "mac":
                    ...
                    target_postbuild = self.xcode_settings.AddImplicitPostbuilds(
                        ...
                        QuoteSpaces(
                            os.path.normpath(
                                os.path.join(gyp_to_build, self.output))),
                        QuoteSpaces(
                            os.path.normpath(
                                os.path.join(gyp_to_build,
                                             self.output_binary))),
                    )
                    if target_postbuild:
        ...
        if postbuilds:
            ...
            # using ":=".
            self.WriteSortedXcodeEnv(self.output,
                                     self.GetSortedXcodePostbuildEnv())
        ...
        if self.is_mac_bundle:
            ...
            self.WriteList([QuoteSpaces(dep) for dep in bundle_deps],
                           "BUNDLE_DEPS")
        ...
        if self.type == "executable":
            ...
            for link_dep in link_deps:
                assert " " not in link_dep, (
                    "Spaces in alink input filenames not supported (%s)" %
                    link_dep)
            if (self.flavor not in ("mac", "openbsd", "netbsd", "win")
            ...
            for link_dep in link_deps:
                assert " " not in link_dep, (
                    "Spaces in module input filenames not supported (%s)" %
                    link_dep)
            if self.toolset == "host" and self.flavor == "android":
                self.WriteDoCmd(
                    [self.output_binary],
            ...
            # Write a stamp line.
            self.WriteDoCmd([self.output_binary],
                            deps,
                            "touch",
                            part_of_all,
                            postbuilds=postbuilds)
        ...
        if self.type in self._INSTALLABLE_TARGETS or self.is_standalone_static_library:
            ...
            if (self.flavor == "mac" and "product_dir" not in spec
    ...
    def WriteMakeRule(
        ...
        if order_only:
            ...
            self.WriteLn("{}: | {}{}".format(" ".join(outputs),
                                             " ".join(inputs), force_append))
        elif len(outputs) == 1:
            # Regular rule, one output: Just write a simple rule.
            self.WriteLn("{}: {}{}".format(outputs[0], " ".join(inputs),
                                           force_append))
        else:
            ...
            self.WriteLn("{}: {}{}".format(intermediate, " ".join(inputs),
                                           force_append))
    ...
    def Pchify(self, path, lang):
        ...
        if "$(" in path:
            path = path.replace("$(obj)/",
                                f"$(obj).{self.toolset}/$(TARGET)/pch-{lang}")
            return path
...
def GenerateOutput(target_list, target_dicts, data, params):
    ...
    def CalculateMakefilePath(build_file, base_name):
        ...
        if options.generator_output:
            output_file = os.path.join(options.depth, options.generator_output,
                                       base_path, base_name)
        base_path = gyp.common.RelativePath(os.path.dirname(build_file),
                                            options.toplevel_dir)
    ...
    if options.generator_output:
        ...
        makefile_path = os.path.join(options.toplevel_dir,
                                     options.generator_output, makefile_name)
    ...
    if flavor == "mac":
        ...
        if CC_target == "clang":
            ...
            CXX_target = GetEnvironFallback(("CXX_target", "CXX"),
                                            "ibm-clang++64")
    ...
    for qualified_target in target_list:
        ...
        build_files.add(
            gyp.common.RelativePath(build_file, options.toplevel_dir))
