...
class NinjaWriter:
    ...
    def ExpandRuleVariables(self, path, root, dirname, source, ext, name):
        ...
        path = path.replace(generator_default_variables["RULE_INPUT_DIRNAME"],
                            dirname)
        path = path.replace(generator_default_variables["RULE_INPUT_PATH"],
                            source)
        path = path.replace(generator_default_variables["RULE_INPUT_EXT"], ext)
        path = path.replace(generator_default_variables["RULE_INPUT_NAME"],
                            name)
        return path
    ...
    def WriteSpec(self, spec, config_name, generator_flags):
        ...
        self.target_rpath = generator_flags.get("target_rpath",
                                                r"\$$ORIGIN/lib/")
    ...
    def WriteActionsRulesCopies(self, spec, extra_sources, prebuild,
        ...
        if "actions" in spec:
            outputs += self.WriteActions(spec["actions"], extra_sources,
                                         prebuild, extra_mac_bundle_resources)
        if "rules" in spec:
        ...
        if self.is_mac_bundle:
            xcassets = self.WriteMacBundleResources(
                extra_mac_bundle_resources + mac_bundle_resources,
                mac_bundle_depends)
            partial_info_plist = self.WriteMacXCassets(xcassets,
                                                       mac_bundle_depends)
    ...
    def WriteActions(self, actions, extra_sources, prebuild,
        ...
        for action in actions:
            ...
            pool = "console" if int(action.get("ninja_use_console",
                                               0)) else None
            rule_name, _ = self.WriteNewNinjaRule(name,
                                                  args,
                                                  description,
                                                  win_shell_flags,
                                                  env,
            ...
            if int(action.get("process_outputs_as_mac_bundle_resources",
                              False)):
    ...
    def WriteRules(
        ...
        for rule in rules:
            ...
            description = self.GenerateDescription(
                ...
                ("%s " + generator_default_variables["RULE_INPUT_PATH"]) %
                name,
            )
            win_shell_flags = (self.msvs_settings.GetRuleShellFlags(rule)
                               if self.flavor == "win" else None)
            pool = "console" if int(rule.get("ninja_use_console", 0)) else None
            rule_name, args = self.WriteNewNinjaRule(name, args, description,
                                                     win_shell_flags, env,
                                                     pool)
            ...
            inputs = [
                self.GypPathToNinja(i, env) for i in rule.get("inputs", [])
            ]
            ...
            if num_inputs > 2 and len(sources) > 2:
                inputs = [
                    self.WriteCollapsedDependencies(rule["rule_name"],
                                                    inputs,
                                                    order_only=prebuild)
                ]
                prebuild = []
            ...
            for source in sources:
                ...
                outputs = [
                    self.ExpandRuleVariables(o, root, dirname, source, ext,
                                             basename) for o in rule["outputs"]
                ]
                ...
                if was_mac_bundle_resource or int(
                        rule.get("process_outputs_as_mac_bundle_resources",
                                 False)):
    ...
    def WriteMacXCassets(self, xcassets, bundle_depends):
        ...
        outputs = []
        outputs.append(
            os.path.join(self.xcode_settings.GetBundleResourceFolder(),
                         "Assets.car"))
    ...
    def WriteMacInfoPlist(self, partial_info_plist, bundle_depends):
        ...
        if partial_info_plist:
            intermediate_plist = self.GypPathToUniqueOutput(
                "merged_info.plist")
            info_plist = self.ninja.build(intermediate_plist,
                                          "merge_infoplist",
    ...
    def WriteSourcesForArch(
        ...
        self.WriteVariableList(
            ...
            "includes",
            [
                QuoteShellArgument("-I" + self.GypPathToNinja(i, env),
                                   self.flavor) for i in include_dirs
            ],
        )
        ...
        if self.flavor == "win":
            ...
            self.WriteVariableList(
                ...
                [
                    QuoteShellArgument("-I" + self.GypPathToNinja(i, env),
                                       self.flavor) for i in midl_include_dirs
                ],
            )
        ...
        for source in sources:
            ...
            if ext in ("cc", "cpp", "cxx"):
                ...
                command = "cc_s"
            elif (self.flavor == "win" and ext in ("asm", "S")
        ...
        if has_rc_source:
            resource_include_dirs = config.get("resource_include_dirs",
                                               include_dirs)
            self.WriteVariableList(
                ninja_file,
                "resource_includes",
    ...
    def WriteLink(self, spec, config_name, config, link_deps, compile_deps):
        ...
        if self.flavor != "mac" or len(self.archs) == 1:
            return self.WriteLinkForArch(self.ninja, spec, config_name, config,
                                         link_deps, compile_deps)
        else:
            ...
            if not self.is_mac_bundle:
                self.AppendPostbuildVariable(extra_bindings, spec, output,
                                             output)
    ...
    def WriteLinkForArch(self,
        ...
        if "dependencies" in spec:
            ...
            for dep in spec["dependencies"]:
                ...
                if linkable:
                    new_deps = []
                    if (self.flavor == "win" and target.component_objs
                            and self.msvs_settings.
        ...
        if self.flavor == "mac":
            ...
            if intermediate_manifest:
                self.WriteVariableList(ninja_file, "intermediatemanifest",
                                       [intermediate_manifest])
        ...
        if self.flavor == "win":
            ...
            library_dirs = [
                "/LIBPATH:" + QuoteShellArgument(
                    self.GypPathToNinja(library_dir), self.flavor)
                for library_dir in library_dirs
            ]
        else:
            library_dirs = [
                QuoteShellArgument("-L" + self.GypPathToNinja(library_dir),
                                   self.flavor) for library_dir in library_dirs
            ]
    ...
    def WriteTarget(self, spec, config_name, config, link_deps, compile_deps):
        ...
        if spec["type"] == "none" or (not link_deps and not extra_link_deps):
            ...
            if (self.flavor not in ("ios", "mac", "netbsd", "openbsd", "win")
            ...
            self.target.binary = self.WriteLink(spec, config_name, config,
                                                link_deps, compile_deps)
        return self.target.binary

    def WriteMacBundle(self, spec, mac_bundle_depends, is_empty):
        assert self.is_mac_bundle
        package_framework = spec["type"] in ("shared_library",
                                             "loadable_module")
    ...
    def AppendPostbuildVariable(self,
                                variables,
                                spec,
                                output,
                                binary,
        ...
        postbuild = self.GetPostbuildCommand(spec, output, binary,
                                             is_command_start)
        if postbuild:
    ...
    def GetPostbuildCommand(self, spec, output, output_binary,
        ...
        postbuilds = gyp.xcode_emulation.GetSpecPostbuildCommands(spec,
                                                                  quiet=True)
        if output_binary is not None:
            postbuilds = self.xcode_settings.AddImplicitPostbuilds(
                ...
                QuoteShellArgument(
                    os.path.normpath(
                        os.path.join(self.base_to_build, output_binary)),
                    self.flavor,
                ),
                postbuilds,
                quiet=True,
    ...
    def ComputeExportEnvString(self, env):
        ...
        for k, v in env:
            export_str.append(
                "export %s=%s;" %
                (k, ninja_syntax.escape(
                    gyp.common.EncodePOSIXShellArgument(v))))
    ...
    def ComputeOutputFileName(self, spec, type=None):
        ...
        extension = "." + extension if extension else DEFAULT_EXTENSION.get(
            type, "")
    ...
    def ComputeOutput(self, spec, arch=None):
        ...
        if (arch is None and self.flavor == "mac"
                and type in ("static_library", "executable", "shared_library",
                             "loadable_module")):
    ...
    def WriteNewNinjaRule(self,
        ...
        if self.flavor == "win":
            args = [
                self.msvs_settings.ConvertVSMacros(arg,
                                                   self.base_to_build,
                                                   config=self.config_name)
                for arg in args
            ]
            ...
            args = [
                gyp.xcode_emulation.ExpandEnvVars(arg, env) for arg in args
            ]
...
def GetDefaultConcurrentLinks():
    ...
    if sys.platform in ("win32", "cygwin"):
        ...
        hard_cap = max(1, int(os.environ.get("GYP_LINK_CONCURRENCY_MAX",
                                             2**32)))
...
def _AddWinLinkRules(master_ninja, embed_manifest):
    ...
    use_separate_mspdbsrv = int(
        os.environ.get("GYP_USE_SEPARATE_MSPDBSRV", "0")) != 0
...
def GenerateOutputForConfig(target_list, target_dicts, data, params,
    ...
    if flavor == "win":
        ...
        cl_paths = gyp.msvs_emulation.GenerateEnvironmentFiles(
            toplevel_build, generator_flags, shared_system_includes,
            OpenOutput)
    ...
    if flavor == "win":
        ...
        master_ninja.variable("ldxx",
                              CommandWithWrapper("LINK", wrappers, ldxx))
        master_ninja.variable("ar", GetEnvironFallback(["AR_target", "AR"],
                                                       ar))
        if flavor != "mac":
            # Mac does not use readelf/nm for .TOC generation, so avoiding polluting
            # the master ninja with extra unused variables.
            master_ninja.variable("nm",
                                  GetEnvironFallback(["NM_target", "NM"], nm))
    ...
    if generator_supports_multiple_toolsets:
        ...
        master_ninja.variable("ar_host",
                              GetEnvironFallback(["AR_host"], ar_host))
        master_ninja.variable("nm_host",
    ...
    if flavor not in ("ios", "mac", "win"):
        ...
        master_ninja.rule(
            ...
            command=
            "$env ./gyp-mac-tool copy-info-plist $in $out $binary $keys",
        )
        master_ninja.rule(
            "merge_infoplist",
