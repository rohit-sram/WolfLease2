...
def NormjoinPath(base_path, rel_path):
  ...
  """
    if rel_path.startswith(
            "$") and not rel_path.startswith("${configuration}"):
        return rel_path
...
def WriteActions(target_name, actions, extra_sources, extra_deps, path_to_gyp,
                 output):
...
def WriteRules(target_name, rules, extra_sources, extra_deps, path_to_gyp,
               output):
  ...
  """
    for rule in rules:
        rule_name = StringToCMakeTargetName(target_name + "__" +
                                            rule["rule_name"])
        ...
        for count, rule_source in enumerate(rule.get("rule_sources", [])):
            ...
            dirs = {
                dir
                for dir in (os.path.dirname(o) for o in outputs) if dir
            }
  ...
  """
    ...
    def __init__(self, target_list):
        ...
        for qualified_target in target_list:
            cmake_target_base_name = CreateCMakeTargetBaseName(
                qualified_target)
...
def WriteTarget(
    ...
    has_sources = (s_sources_name or c_sources_name or cxx_sources_name
                   or linkable_sources_name or other_sources_name)
    ...
    if library_dirs is not None:
        ...
        for library_dir in library_dirs:
            output.write(" ")
            output.write(NormjoinPath(path_from_cmakelists_to_gyp,
                                      library_dir))
            output.write("\n")
        output.write(")\n")
    ...
    SetTargetProperty(output, cmake_target_name, "EXCLUDE_FROM_ALL",
                      exclude_from_all)
    for extra_target_name in extra_deps:
        SetTargetProperty(output, extra_target_name, "EXCLUDE_FROM_ALL",
                          exclude_from_all)
    ...
    if target_type != "none":
        ...
        if not c_sources and not cxx_sources:
            SetTargetProperty(output, cmake_target_name, "LINKER_LANGUAGE",
                              ["C"])
        ...
        if target_output_directory is None:
            if target_type in ("executable", "loadable_module"):
                ...
                target_output_directory = os.path.join(target_output_directory,
                                                       base_path)
        ...
        if includes:
            ...
            SetVariableList(
                ...
                includes_name,
                [
                    NormjoinPathForceCMakeSource(path_from_cmakelists_to_gyp,
                                                 include)
        ...
        if defines is not None:
            SetTargetProperty(output, cmake_target_name, "COMPILE_DEFINITIONS",
                              defines, ";")
        ...
        if (not cflags_c or not c_sources) and (not cflags_cxx
                                                or not cxx_sources):
            SetTargetProperty(output, cmake_target_name, "COMPILE_FLAGS",
                              cflags, " ")
            ...
            if s_sources and cflags:
                SetFilesProperty(output, s_sources_name, "COMPILE_FLAGS",
                                 cflags, " ")
            ...
            if c_sources and (cflags or cflags_c):
                ...
                flags.extend(cflags)
                flags.extend(cflags_c)
                SetFilesProperty(output, c_sources_name, "COMPILE_FLAGS",
                                 flags, " ")
            ...
            if cxx_sources and (cflags or cflags_cxx):
                ...
                flags.extend(cflags)
                flags.extend(cflags_cxx)
                SetFilesProperty(output, cxx_sources_name, "COMPILE_FLAGS",
                                 flags, " ")
    ...
    linkable = target_type in ("executable", "loadable_module",
                               "shared_library")
    ...
    if linkable:
        external_libs = [
            lib for lib in spec.get("libraries", []) if len(lib) > 0
        ]
...
def GenerateOutputForConfig(target_list, target_dicts, data, params,
    ...
    build_dir = os.path.normpath(
        os.path.join(generator_dir, output_dir, config_to_use))
