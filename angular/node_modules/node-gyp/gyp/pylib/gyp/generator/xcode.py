...
sys.exit(subprocess.call(sys.argv[1:]))" """
    ...
    def Finalize2(self, xcode_targets, xcode_target_to_target_dict):
        ...
        for bf_tgt in self.build_file_dict["targets"]:
            if int(bf_tgt.get("xcode_create_dependents_test_runner", 0)):
                ...
                xcode_target = xcode_targets[qualified_target]
                if isinstance(xcode_target,
                              gyp.xcodeproj_file.PBXAggregateTarget):
    ...
    def Write(self):
        ...
        (output_fd,
         new_pbxproj_path) = tempfile.mkstemp(suffix=".tmp",
                                              prefix="project.pbxproj.gyp.",
                                              dir=self.path)
...
def CalculateGeneratorInputInfo(params):
    ...
    if params.get("flavor") == "ninja":
        generator_dir = os.path.relpath(params["options"].generator_output
                                        or ".")
...
def GenerateOutput(target_list, target_dicts, data, params):
    ...
    if ninja_wrapper:
        (target_list, target_dicts,
         data) = gyp.xcode_ninja.CreateWrapper(target_list, target_dicts, data,
                                               params)

    options = params["options"]
    generator_flags = params.get("generator_flags", {})
    parallel_builds = generator_flags.get("xcode_parallel_builds", True)
    serialize_all_tests = generator_flags.get("xcode_serialize_all_test_runs",
                                              True)
    ...
    skip_excluded_files = not generator_flags.get("xcode_list_excluded_files",
                                                  True)
    xcode_projects = {}
    for build_file, build_file_dict in data.items():
        (build_file_root, build_file_ext) = os.path.splitext(build_file)
        ...
        if upgrade_check_project_version:
            project_attributes[
                "LastUpgradeCheck"] = upgrade_check_project_version
            project_attributes[
                "LastTestingUpgradeCheck"] = upgrade_check_project_version
            project_attributes[
    ...
    for qualified_target in target_list:
        [build_file, target_name,
         toolset] = gyp.common.ParseQualifiedTarget(qualified_target)
        ...
        # that case.
        if type != "none" and (spec_actions
                               or spec_rules) and not ninja_wrapper:
        ...
        for action in spec_actions:
            ...
            if message:
                message = "echo note: " + gyp.common.EncodePOSIXShellArgument(
                    message)
            else:
                message = ""
        ...
        for rule in spec_rules:
            ...
            for rule_source in rule.get("rule_sources", []):
                rule_source_dirname, rule_source_basename = posixpath.split(
                    rule_source)
                (rule_source_root,
                 rule_source_ext) = posixpath.splitext(rule_source_basename)
            ...
            if len(concrete_outputs_all) > 0:
                ...
                makefile_name = "%s.make" % re.sub(
                    "[^a-zA-Z0-9_]", "_", "{}_{}".format(
                        target_name, rule["rule_name"]))
                ...
                for (rule_source, concrete_outputs, message, action) in zip(
                    ...
                    for concrete_output_index, concrete_output in enumerate(
                        ...
                        concrete_output_dir = posixpath.dirname(
                            concrete_output)
                        if (concrete_output_dir and concrete_output_dir
                                not in concrete_output_dirs):
...
""" % makefile_name)
        ...
        if is_bundle:
            for resource in tgt_mac_bundle_resources:
                (resource_root,
                 resource_extension) = posixpath.splitext(resource)
        ...
        for postbuild in spec.get("postbuilds", []):
            ...
            ssbp = gyp.xcodeproj_file.PBXShellScriptBuildPhase({
                ...
                "name":
                'Postbuild "' + postbuild["postbuild_name"] + '"',
                "shellScript":
                script,
                "showEnvVarsInLog":
                0,
        ...
        if "libraries" in spec:
            for library in spec["libraries"]:
                ...
                if library_dir not in xcode_standard_library_dirs and (
                        ...
                        or library_dir
                        not in xct.GetBuildSetting(_library_search_paths_var)):
                    xct.AppendBuildSetting(_library_search_paths_var,
                                           library_dir)
        ...
        for configuration_name in configuration_names:
            ...
            for library_dir in configuration.get("library_dirs", []):
                if library_dir not in xcode_standard_library_dirs and (
                        ...
                        or library_dir not in xcbc.GetBuildSetting(
                            _library_search_paths_var)):
                    xcbc.AppendBuildSetting(_library_search_paths_var,
                                            library_dir)
