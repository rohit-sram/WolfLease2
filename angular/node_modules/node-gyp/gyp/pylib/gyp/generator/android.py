...
class AndroidMkWriter:
    ...
    def WriteActions(self, actions, extra_sources, extra_outputs):
        ...
        for action in actions:
            name = make.StringToMakefileVariable("{}_{}".format(
                self.relative_target, action["action_name"]))
    ...
    def WriteRules(self, rules, extra_sources, extra_outputs):
        ...
        for rule in rules:
            if len(rule.get("rule_sources", [])) == 0:
                continue
            name = make.StringToMakefileVariable("{}_{}".format(
                self.relative_target, rule["rule_name"]))
    ...
    def WriteCopies(self, copies, extra_outputs):
        ...
        variable = make.StringToMakefileVariable(self.relative_target +
                                                 "_copies")
        outputs = []
        for copy in copies:
            for path in copy["files"]:
                ...
                output = Sourceify(
                    self.LocalPathify(
                        os.path.join(copy["destination"], filename)))
        ...
        self.WriteLn("{} = {}".format(variable,
                                      " ".join(map(make.QuoteSpaces,
                                                   outputs))))
    ...
    def WriteSources(self, spec, configs, extra_sources):
        ...
        for source in sources:
            ...
            if "$(gyp_shared_intermediate_dir)" in source:
                ...
                local_files.append(
                    os.path.normpath(os.path.join(self.path, source)))
        ...
        origin_src_dirs = map(Sourceify, map(self.LocalPathify,
                                             origin_src_dirs))
    ...
    def FilterLibraries(self, libraries):
        ...
        for libs in libraries:
            ...
            for lib in libs.split():
                ...
                # build system.
                if (lib == "-lc" or lib == "-lstdc++" or lib == "-lm"
...
def PerformBuild(data, configurations, params):
    ...
    makefile = os.path.abspath(
        os.path.join(options.toplevel_dir, "GypAndroid.mk"))
    env = dict(os.environ)
    env["ONE_SHOT_MAKEFILE"] = makefile
    arguments = [
        "make", "-C", os.environ["ANDROID_BUILD_TOP"], "gyp_all_modules"
    ]
