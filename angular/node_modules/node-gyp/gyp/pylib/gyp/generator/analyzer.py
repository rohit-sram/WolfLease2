  ...
  targets: see file description for details."""
    ...
    if there is a parse error."""
        ...
        try:
            ...
            raise Exception("Unable to parse config file " + config_path +
                            str(e))
...
def _GenerateTargets(data, target_list, target_dicts, toplevel_dir, files,
  ...
  |toplevel_dir| is the root of the source tree."""
    ...
    while len(targets_to_visit) > 0:
        ...
        created_target, target = _GetOrCreateTargetByName(
            name_to_target, target_name)
        if created_target:
            roots.add(target)
        elif target.visited:
            continue
...
def _DoesTargetDependOnMatchingTargets(target):
  ...
  target: the Target to look for."""
    if target.match_status == MATCH_STATUS_DOESNT_MATCH:
        return False
    if (target.match_status
            in {MATCH_STATUS_MATCHES, MATCH_STATUS_MATCHES_BY_DEPENDENCY}):
        return True
...
def _AddCompileTargets(target, roots, add_if_no_ancestor, result):
  ...
  result: targets that need to be built are added here."""
    ...
    if target.in_roots and (target.is_executable or
                            (not target.added_to_compile_targets and
                             (add_if_no_ancestor or target.requires_build)) or
                            (target.is_static_library and add_if_no_ancestor
...
def _WriteOutput(params, **values):
    ...
    output_path = params.get("generator_flags",
                             {}).get("analyzer_output_path", None)
    if not output_path:
...
class TargetCalculator:
    ...
    def find_matching_test_target_names(self):
        ...
        if test_target_names_contains_all:
            test_targets = list(
                set(test_targets_no_all) | set(self._root_targets))
        else:
        ...
        matching_test_targets_contains_all = test_target_names_contains_all and set(
            matching_test_targets) & set(self._root_targets)
    ...
    def find_matching_compile_target_names(self):
        ...
        if "all" in self._supplied_target_names():
            supplied_targets = list(
                set(supplied_targets) | set(self._root_targets))
...
def GenerateOutput(target_list, target_dicts, data, params):
    ...
    try:
        ...
        if not config.files:
            raise Exception(
                "Must specify files to analyze via config_path generator "
                "flag")
        ...
        if _WasGypIncludeFileModified(params, config.files):
            result_dict = {
                ...
                all_changed_string,
                "test_targets":
                list(config.test_target_names),
                "compile_targets":
                list(config.additional_compile_target_names
        ...
        result_dict = {
            ...
            if found_at_least_one_target else no_dependency_string,
            "compile_targets":
            list(set(compile_target_names) | set(test_target_names)),
        }
        if calculator.invalid_targets:
            result_dict["invalid_targets"] = calculator.invalid_targets
        _WriteOutput(params, **result_dict)
