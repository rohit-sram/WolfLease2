...
class VisualStudioVersion:
    ...
    def __init__(
        ...
        compatible_sdks.sort(key=lambda v: float(v.replace("v", "")),
                             reverse=True)
    ...
    environment."""
        ...
        is_host_arch_x64 = (os.environ.get("PROCESSOR_ARCHITECTURE") == "AMD64"
                            or os.environ.get("PROCESSOR_ARCHITEW6432")
                            == "AMD64")

        # For VS2017 (and newer) it's fairly easy
        if self.short_name >= "2017":
            script_path = JoinPath(self.path, "VC", "Auxiliary", "Build",
                                   "vcvarsall.bat")
        ...
        if target_arch == "x86":
            if (self.short_name >= "2013" and self.short_name[-1] != "e"
                ...
                return [
                    JoinPath(self.path, "Common7", "Tools", "vsvars32.bat")
                ]
...
def _RegistryQueryBase(sysdir, key, value):
  ...
  """
    ...
    cmd = [
        os.path.join(os.environ.get("WINDIR", ""), sysdir, "reg.exe"), "query",
        key
    ]
    if value:
        cmd.extend(["/v", value])
    ...
    for version in versions_to_check:
        ...
        for index in range(len(keys)):
            ...
            if not force_express and os.path.exists(full_path):
                # Add this one.
                versions.append(
                    _CreateVersion(version_to_year[version],
                                   os.path.join(path, "..", "..")))
            # Check for express.
            elif glob.glob(express_path):
                # Add this one.
                versions.append(
                    _CreateVersion(version_to_year[version] + "e",
                                   os.path.join(path, "..", "..")))
...
def SelectVisualStudioVersion(version="auto", allow_fallback=True):
  ...
  """
    ...
    version_map = {
        "auto":
        ("17.0", "16.0", "15.0", "14.0", "12.0", "10.0", "9.0", "8.0", "11.0"),
        "2005": ("8.0", ),
        "2005e": ("8.0", ),
        "2008": ("9.0", ),
        "2008e": ("9.0", ),
    ...
    version = str(version)
    versions = _DetectVisualStudioVersions(version_map[version], "e"
                                           in version)
    if not versions:
