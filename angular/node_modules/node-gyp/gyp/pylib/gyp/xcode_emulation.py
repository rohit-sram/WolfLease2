...
class XcodeArchsDefault:
  ...
  """
    ...
    values present in VALID_ARCHS are kept)."""
        expanded_archs = self._ExpandArchs(archs or self._default, sdkroot
                                           or "")
        if valid_archs:
            filtered_archs = []
  ...
  for performance reason."""
    ...
    if xcode_version < "0500":
        ...
        XCODE_ARCHS_DEFAULT_CACHE = XcodeArchsDefault(
            ...
            XcodeArchsVariableMapping(["armv7", "armv7s"],
                                      ["armv7", "armv7s", "arm64"]),
        )
    else:
        XCODE_ARCHS_DEFAULT_CACHE = XcodeArchsDefault(
...
class XcodeSettings:
    ...
    def __init__(self, spec):
        ...
        for configname, config in configs.items():
            ...
            if self.xcode_settings[configname].get(
    ...
    def IsIosFramework(self):
        return self.spec["type"] == "shared_library" and self._IsBundle(
        ) and self.isIOS
    ...
    Chromium.app/Contents or Foo.bundle/Versions/A. Only valid for bundles."""
        ...
        if self.spec["type"] == "shared_library":
            return os.path.join(self.GetWrapperName(), "Versions",
    ...
    compilations."""
        ...
        if self._Test("GCC_GENERATE_DEBUGGING_SYMBOLS", "YES", default="YES"):
            dbg_format = self._Settings().get("DEBUG_INFORMATION_FORMAT",
                                              "dwarf")
            if dbg_format == "dwarf":
                ...
                raise NotImplementedError("Unknown debug format %s" %
                                          dbg_format)
        ...
        if not gyp.common.CrossCompileRequested():
            ...
            if archs[0] in ("i386", "x86_64"):
                if self._Test("GCC_ENABLE_SSE3_EXTENSIONS",
                              "YES",
                              default="NO"):
                    cflags.append("-msse3")
                if self._Test("GCC_ENABLE_SUPPLEMENTAL_SSE3_INSTRUCTIONS",
                              "YES",
                              default="NO"):
                    cflags.append("-mssse3")  # Note 3rd 's'.
                if self._Test("GCC_ENABLE_SSE41_EXTENSIONS",
                              "YES",
                              default="NO"):
                    cflags.append("-msse4.1")
                if self._Test("GCC_ENABLE_SSE42_EXTENSIONS",
                              "YES",
                              default="NO"):
    ...
    def GetCflagsCC(self, configname):
        ...
        for flag in self._Settings().get("OTHER_CPLUSPLUSFLAGS",
                                         ["$(inherited)"]):
    ...
    def GetInstallNameBase(self):
        ...
        install_base = self.GetPerTargetSetting(
            ...
            if self._IsBundle() else "/usr/local/lib",
        )
        return install_base
    ...
    def GetInstallName(self):
        ...
        if "$" in install_name:
            ...
            if self._IsBundle():
                ...
                install_name = install_name.replace("$(WRAPPER_NAME)",
                                                    self.GetWrapperName())
                install_name = install_name.replace("$(PRODUCT_NAME)",
                                                    self.GetProductName())
            else:
                assert "$(WRAPPER_NAME)" not in install_name
                assert "$(PRODUCT_NAME)" not in install_name

            install_name = install_name.replace("$(EXECUTABLE_PATH)",
    ...
    """
        ...
        self._Appendf(ldflags, "DYLIB_COMPATIBILITY_VERSION",
        ...
        if "ORDER_FILE" in self._Settings():
            ldflags.append("-Wl,-order_file " + "-Wl," +
        ...
        is_extension = self._IsIosAppExtension(
        ) or self._IsIosWatchKitExtension()
    ...
    before the actual postbuilds run."""
        ...
        result = []
        if self._Test("DEPLOYMENT_POSTPROCESSING", "YES",
                      default="NO") and self._Test(
                          "STRIP_INSTALLED_PRODUCT", "YES", default="NO"):

            default_strip_style = "debugging"
            if (self.spec["type"] == "loadable_module"
                    or self._IsIosAppExtension()) and self._IsBundle():
                default_strip_style = "non-global"
            elif self.spec["type"] == "executable":
                default_strip_style = "all"

            strip_style = self._Settings().get("STRIP_STYLE",
                                               default_strip_style)
            strip_flags = {
                "all": "",
                "non-global": "-x",
            ...
            if explicit_strip_flags:
                strip_flags += " " + _NormalizeEnvVarReferences(
    ...
    as postbuilds before the actual postbuilds run."""
        ...
        result = []
        if (self._Test("GCC_GENERATE_DEBUGGING_SYMBOLS", "YES", default="YES")
                and self._Test("DEBUG_INFORMATION_FORMAT",
                               "dwarf-with-dsym",
                               default="dwarf")
            ...
            if not quiet:
                result.append("echo DSYMUTIL\\(%s\\)" %
                              self.spec["target_name"])
            result.append("dsymutil {} -o {}".format(output_binary,
                                                     output + ".dSYM"))

        self.configname = None
        return result

    def _GetTargetPostbuilds(self,
                             configname,
                             output,
                             output_binary,
                             quiet=False):
    ...
    build."""
        ...
        if self._IsXCTest():
            ...
            xctest_destination = os.path.join(test_host, "PlugIns",
                                              product_name)
        ...
        if self._IsXCTest():
            ...
            for framework in frameworks:
                ...
                destination = os.path.join(frameworks_dir,
        ...
        if identity not in XcodeSettings._codesigning_key_cache:
            ...
            for line in output.splitlines():
                if identity in line:
                    ...
                    assert identity not in cache or fingerprint == cache[
                        identity], (
                            "Multiple codesigning fingerprints for identity: %s"
                            % identity)
                    XcodeSettings._codesigning_key_cache[
                        identity] = fingerprint
        return XcodeSettings._codesigning_key_cache.get(identity, "")

    def AddImplicitPostbuilds(self,
                              configname,
                              output,
                              output_binary,
                              postbuilds=[],
    ...
    |postbuilds|."""
        ...
        pre = self._GetTargetPostbuilds(configname, output, output_binary,
                                        quiet)
    ...
    def _AdjustLibrary(self, library, config_name=None):
        if library.endswith(".framework"):
            l_flag = "-framework " + os.path.splitext(
                os.path.basename(library))[0]
        else:
    ...
    """
        libraries = [
            self._AdjustLibrary(library, config_name) for library in libraries
        ]
        return libraries
    ...
    def GetExtraPlistItems(self, configname=None):
        ...
        if configname not in XcodeSettings._plist_cache:
            ...
            sdk_version = self._GetSdkVersionInfoItem(sdk_root,
                                                      "--show-sdk-version")
            ...
            if self.isIOS:
                cache["MinimumOSVersion"] = self.xcode_settings[
...
class MacPrefixHeader:
  ...
  * If GCC_PRECOMPILE_PREFIX_HEADER is present too, then the prefix header is
  ...
  """
    ...
    def __init__(self, xcode_settings, gyp_path_to_build_path,
    ...
    """
        ...
        return [
            ...
            (self._Gch("mm",
                       arch), "-x objective-c++-header", "mm", self.header),
        ]
...
def XcodeVersion():
    ...
    version[0] = version[0].zfill(
        2)  # Add a leading zero if major is one digit
...
def IsMacBundle(flavor, spec):
  ...
  resources into that directory."""
    is_mac_bundle = (int(spec.get("mac_xctest_bundle", 0)) != 0
                     or int(spec.get("mac_xcuitest_bundle", 0)) != 0 or
...
def GetMacInfoPlist(product_dir, xcode_settings, gyp_path_to_build_path):
  ...
  * |extra_env| is a dict of env variables that should be exported when
  ...
  """
    ...
    if (xcode_settings.GetPerTargetSetting("INFOPLIST_PREPROCESS",
                                           default="NO") == "YES"):
...
def GetSortedXcodeEnv(xcode_settings,
                      built_products_dir,
                      srcroot,
                      configuration,
                      additional_settings=None):
    env = _GetXcodeEnv(xcode_settings, built_products_dir, srcroot,
                       configuration, additional_settings)
  ...
  to build for iOS devices and use correct architectures for those builds."""
    for target_dict in targets.values():
        ...
        for config_name, simulator_config_dict in dict(configs).items():
            ...
            if toolset == "target":
                simulator_config_dict["xcode_settings"][
