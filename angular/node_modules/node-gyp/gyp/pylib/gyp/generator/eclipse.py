...
for dirname in [
        "INTERMEDIATE_DIR", "PRODUCT_DIR", "LIB_DIR", "SHARED_LIB_DIR"
]:
...
def GetAllDefines(target_list, target_dicts, data, config_name, params,
  ...
  """
    ...
    if compiler_path:
        command = shlex.split(compiler_path)
        command.extend(["-E", "-dM", "-"])
        cpp_proc = subprocess.Popen(args=command,
                                    cwd=".",
                                    stdin=subprocess.PIPE,
                                    stdout=subprocess.PIPE)
...
def GenerateClasspathFile(target_list, target_dicts, toplevel_dir,
  ...
  .java and .jar files used as action inputs."""
    ...
    AddElements("src",
                GetJavaSourceDirs(target_list, target_dicts, toplevel_dir))
    # Include the standard JRE container and a dummy out folder
    AddElements("con", ["org.eclipse.jdt.launching.JRE_CONTAINER"])
    # Include a dummy out folder so that Eclipse doesn't use the default /bin
    # folder in the root of the project.
    AddElements("output",
                [os.path.join(toplevel_build, ".eclipse-java-build")])
    ...
    for target_name in target_list:
        ...
        for action in target.get("actions", []):
            for input_ in action["inputs"]:
                if os.path.splitext(
                        input_)[1] == ".jar" and not input_.startswith("$"):
                    if os.path.isabs(input_):
                        yield input_
                    else:
                        yield os.path.join(os.path.dirname(target_name),
                                           input_)
...
def GetJavaSourceDirs(target_list, target_dicts, toplevel_dir):
    ...
    for target_name in target_list:
        ...
        for action in target.get("actions", []):
            for input_ in action["inputs"]:
                if os.path.splitext(
                        input_)[1] == ".java" and not input_.startswith("$"):
                    ...
                    parent_search = dir_
                    while os.path.basename(parent_search) not in [
                            "src", "java"
                    ]:
